...
def obtener_categoria(categoria_id: int, db: Session = Depends(get_db), current_user = Depends(require_roles(["admin", "banda", "secretaria"]))):
...
def crear_categoria(data: CategoriaCreate, db: Session = Depends(get_db), current_user = Depends(require_roles(["admin", "banda"]))):
...
def actualizar_categoria(categoria_id: int, data: CategoriaUpdate, db: Session = Depends(get_db), current_user = Depends(require_roles(["admin", "banda"]))):
...
def eliminar_categoria(categoria_id: int, db: Session = Depends(get_db), current_user = Depends(require_roles(["admin"]))):
...
def listar_instrumentos(
    ...
    solo_disponibles: bool = Query(False), categoria_id: Optional[int] = Query(None),
    db: Session = Depends(get_db), current_user = Depends(require_roles(["admin", "banda", "secretaria", "titular"]))
):
...
def instrumentos_disponibles(db: Session = Depends(get_db), current_user = Depends(require_roles(["admin", "banda", "secretaria"]))):
...
def obtener_instrumento(instrumento_id: int, db: Session = Depends(get_db), current_user = Depends(require_roles(["admin", "banda", "secretaria"]))):
...
def crear_instrumento(data: InstrumentoCreate, db: Session = Depends(get_db), current_user = Depends(require_roles(["admin", "banda"]))):
    try:
        return service.create_instrumento(db, data.model_dump(), current_user)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))

@router.put("/instrumentos/{instrumento_id}", response_model=InstrumentoResponse)
def actualizar_instrumento(instrumento_id: int, data: InstrumentoUpdate, db: Session = Depends(get_db), current_user = Depends(require_roles(["admin", "banda"]))):
    try:
        instrumento = service.update_instrumento(db, instrumento_id, data.model_dump(exclude_unset=True), current_user)
        if not instrumento:
            raise HTTPException(status_code=404, detail="Instrumento no encontrado")
        return instrumento
    except ValueError as e:
...
def eliminar_instrumento(instrumento_id: int, db: Session = Depends(get_db), current_user = Depends(require_roles(["admin"]))):
    try:
        if not service.delete_instrumento(db, instrumento_id, current_user):
            raise HTTPException(status_code=404, detail="Instrumento no encontrado")
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    ...
    return [
        {
            ...
            "dias_prestado": (date.today() - p.fecha_prestamo).days if p.fecha_prestamo else 0 # <--- CORREGIDO
        }
        for p in prestamos
    ]
    
@router.get("/prestamos/{prestamo_id}", response_model=PrestamoInstrumentoResponse)
def obtener_prestamo(prestamo_id: int, db: Session = Depends(get_db), current_user = Depends(require_roles(["admin", "banda", "secretaria"]))):
    ...
    return {
        ...
        "estado_al_devolver": prestamo.estado_al_devolver,
...
def registrar_prestamo(data: PrestamoInstrumentoCreate, db: Session = Depends(get_db), current_user = Depends(require_roles(["admin", "banda"]))):
    try:
        prestamo = service.create_prestamo(db, data.model_dump(), current_user)
        if not prestamo:
...
def devolver_instrumento(prestamo_id: int, data: DevolucionCreate, db: Session = Depends(get_db), current_user = Depends(require_roles(["admin", "banda"]))):
    try:
        prestamo = service.devolver_instrumento(db, prestamo_id, data.model_dump(), current_user)
        if not prestamo:
            raise HTTPException(status_code=404, detail="Préstamo no encontrado")
        return prestamo
    except ValueError as e:
...
def actualizar_prestamo(prestamo_id: int, data: PrestamoInstrumentoUpdate, db: Session = Depends(get_db), current_user = Depends(require_roles(["admin", "banda"]))):
...
def historial_instrumento(instrumento_id: int, db: Session = Depends(get_db), current_user = Depends(require_roles(["admin", "banda", "secretaria"]))):
...
def obtener_estadisticas(db: Session = Depends(get_db), current_user = Depends(require_roles(["admin", "banda"]))):
